...
bool Check_valid(string user_ch) {
	if (user_ch == "rock" || user_ch == "paper" || user_ch == "scissors") return true;
	return false;
...
void check_win(string ch, string user_ch) {
	...
	string winner;
	for (int i = 0; i < 2; i++) {
		if (arr[i] == "rock" && (arr[i + 1] == "scissors" || arr[i - 1] == "scissors")) {
			winner = arr[i];
			break;
		...
		else if (arr[i] == "scissors" && (arr[i + 1] == "paper" || arr[i - 1] == "paper")) {
			winner = arr[i];
			break;
		}
...
void rock_paper_scissors() {
	...
	while (cont == true) {
		...
		do {
			...
			cout << "Your turn!! enter one of those: rock, paper, or scissors: " ;
			cin >> user_ch;
